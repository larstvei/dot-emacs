...
** Font
   ...
   (cond ((member "SauceCodePro Nerd Font" (font-family-list))
...
** Terminal emulator
   ...
   [[https://github.com/dakra/ghostel][Ghostel]] is a terminal emulator for Emacs building on [[https://mitchellh.com/writing/libghostty-is-coming][libghostty]], which is
   the embeddable library core of [[https://ghostty.org/][Ghostty]].

   Inspired by [[https://github.com/torenord/.emacs.d][torenord]], I maintain quick access to ghostel buffers with
   bindings ~M-1~ to ~M-9~. In addition, the ~C-z~ toggles between the last visited
   terminal, and the last visited non-terminal buffer.

   Fresh terminal buffers spawn with the directory given by ~vc-root-dir~ if it
   ...
   (use-package ghostel
     ...
     (defun toggle-ghostel (&optional n)
       (interactive)
       (setq ghostels (seq-filter 'buffer-live-p ghostels))
       (let ((default-directory (or (vc-root-dir) default-directory)))
        (cond ((numberp n) (push (ghostel n) ghostels))
              ((null ghostels) (push (ghostel 1) ghostels))
              ((seq-contains-p ghostels (current-buffer))
               (switch-to-buffer (car (seq-difference (buffer-list) ghostels))))
              (t (switch-to-buffer (car (seq-intersection (buffer-list) ghostels)))))))

     :bind (:map custom-bindings-map
                 ("C-z" . toggle-ghostel)
                 ("M-1" . (lambda () (interactive) (toggle-ghostel 1)))
                 ("M-2" . (lambda () (interactive) (toggle-ghostel 2)))
                 ("M-3" . (lambda () (interactive) (toggle-ghostel 3)))
                 ("M-4" . (lambda () (interactive) (toggle-ghostel 4)))
                 ("M-5" . (lambda () (interactive) (toggle-ghostel 5)))
                 ("M-6" . (lambda () (interactive) (toggle-ghostel 6)))
                 ("M-7" . (lambda () (interactive) (toggle-ghostel 7)))
                 ("M-8" . (lambda () (interactive) (toggle-ghostel 8)))
                 ("M-9" . (lambda () (interactive) (toggle-ghostel 9)))))
